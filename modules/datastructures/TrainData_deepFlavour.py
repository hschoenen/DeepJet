...
class TrainData_DF_Run2(TrainData):
    ...
    def convertFromSourceFile(self, filename, weighterobjects, istraining):
        ...
        x_global = uproot_tree_to_numpy(urfile,self.global_branches,1,self.nsamples,flat = True)
        x_cpf = uproot_tree_to_numpy(urfile,self.cpf_branches,self.n_cpf,self.nsamples,flat = False)
        x_npf = uproot_tree_to_numpy(urfile,self.npf_branches,self.n_npf,self.nsamples,flat = False)
        x_vtx = uproot_tree_to_numpy(urfile,self.vtx_branches,self.n_vtx,self.nsamples,flat = False)
    ...
    def writeOutPrediction(self, predicted, features, truth, weights, outfilename, inputfile):
        ...
        #files["tree"]
        files["tree"].show()
        out_numpy = np.core.records.fromarrays(np.vstack( (predicted[0].transpose(),truth[0].transpose(), features[0][:,0:2].transpose() ) ), names='prob_isB, prob_isBB, prob_isLeptB, prob_isC, prob_isUDS, prob_isG, isB, isBB, isLeptB, isC, isUDS, isG, jet_pt, jet_eta')
        print('saving to', outfilename)
        np.save(outfilename.strip('.root')+'.npy', out_numpy)
