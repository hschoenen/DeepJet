...
def fgsm_attack_flavour(epsilons=[0.01,0.01,0.01],sample=None,targets=None,thismodel=None,thiscriterion=None,reduced=True, dev=torch.device("cpu"), restrict_impact=-1, epsilon_factors=None):
    if epsilons == [0,0,0]:
        return sample
    glob, cpf, npf, vtx = sample
    y = targets
    ...
    with torch.no_grad():
        ...
        # make sure the true jet flavour is available
        if (epsilon_vector==0).nonzero().size(dim=0)>0:
